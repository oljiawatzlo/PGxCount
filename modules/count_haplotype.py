...
class HaplotypeCounter:
    @staticmethod
    def count_haplotype(masterdata):
        """Count haplotype occurrences in the given masterdata.

        Args:
            masterdata (list): List of samples with gene details.

        Returns:
            dict: A dictionary containing haplotype counts by gender.
        """
        data = masterdata
        result_list = []
...
class HaplotypeFormatter:
    @staticmethod
    def format_haplotype(data):
        """Format haplotype data for further processing.

        Args:
            data (dict): Diplotype data by gender.

        Returns:
            list: Formatted haplotype data.
        """
        ...
        for index, row in unique_combinations.iterrows():
            ...
            filtered_df = df_merge[(df_merge['sample_id'] == sample_id) & (df_merge['gene'] == gene)].drop_duplicates()
            ...
            for hap in unique_haplotypes:
                ...
                filtered_hap_df = filtered_df[filtered_df['haplotype'] == hap].copy() 

                # Define the custom sorting order
                custom_order = ['female', 'male', 'all']
                # Convert the 'cou_gender' column to a Categorical data type with custom order
                filtered_hap_df['Category'] = pd.Categorical(filtered_hap_df['cou_gender'], categories=custom_order, ordered=True)

                # Sort the DataFrame by the 'cou_gender' column
                filtered_hap_df = filtered_hap_df.sort_values(by='Category')

                # Reset the index if needed
                filtered_hap_df = filtered_hap_df.reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    data = HaplotypeCounter.count_haplotype(master_data)
    
    with open('.out/count_haplotype/count_haplotype.json', 'w') as file:
        json.dump(data, file, indent=4)
        
    result = HaplotypeFormatter.format_haplotype(data)
    
    with open('report/pgx_count_haplotype.json', 'w') as file:
